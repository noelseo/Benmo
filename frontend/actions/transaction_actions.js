...
export const createATransaction = (transaction, user) => dispatch => ( 
    TransactionApiUtil.createATransaction(transaction, user) 
        .then((payload) => dispatch(receiveAllTransactions(payload)))
)

// 1. if this is successful,
// 2. it dispatches the below action
//once the transaction is created in the db, we want to get the list of ALL transactions
